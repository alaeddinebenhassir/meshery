...
kubeadm reset --cleanup-tmp-dir -f && iptables -F
kubeadm init --pod-network-cidr=10.244.0.0/16
#ip link set weave down && ip link delete weave type bridge
#ip link set cni0 down && ip link delete cni0 type bridge
mkdir -p $HOME/.kube
...
# manual deployement of nginx controller 
...
#kubectl expose deployment nginx-ingress --port=80 --type=LoadBalancer  --namespace=nginx-ingress

#### 
# expose the deployement  on port 80 and 443 
####



#kubectl apply -f https://raw.githubusercontent.com/kubernetes/ingress-nginx/controller-v1.2.0/deploy/static/provider/cloud/deploy.yaml
# the full manifest file is missing something !
